...
class BluetoothPrinterService {
  ...
  static Future<bool> connect(BluetoothDevice device) async {
    try {
      _log(
        'CONNECT',
        message:
            'Connecting to ${device.name ?? "Unknown"} (${device.address ?? "Unknown"})...',
      );
      ...
      final connected = await _printer
          .connect(device)
  ...
  static Future<bool> printEventQr(
    String eventId,
    String namaAcara,
    String tanggalAcara,
    String kodeQr,
    ...
    _log(
      'PRINT',
      message: 'EVENT_ID: $eventId, CONNECTED: ${await _printer.isConnected}',
    );
    ...
    try {
      ...
      _log(
        'PRINT',
        status: 'FAILED',
        message: 'Print timeout after 15 seconds',
      );
      return false;
    } catch (e) {
